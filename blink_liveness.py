...
BLINK_RATIO_THRESHOLD = 7.5
blink_verification_completed = False
...
def check_blink(image):
    global blink_verification_completed#, headpos_verification_completed
    ...
    for face in faces:
        ...
        if blink_ratio > BLINK_RATIO_THRESHOLD:
            ...
            blink_verification_completed = True
            return image_copy_2, True

        else:
            return image_copy_2, False
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for face in faces:
            ...
            if blink_ratio > BLINK_RATIO_THRESHOLD:
                #Blink detected! Do Something!
                cv2.putText(frame_copy,"BLINKING",(10,50), cv2.FONT_HERSHEY_SIMPLEX,
                            2,(255,255,255),2,cv2.LINE_AA)

        cv2.imshow('BlinkDetector', frame_copy)
        key = cv2.waitKey(1)
        if key == 27:
            break
